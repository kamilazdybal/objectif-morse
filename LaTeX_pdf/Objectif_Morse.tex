...
While this outputs are coming we're going to access the file \texttt{/dev/ttyACM0} to capture each one of them and then our code is going to make sense out of that data, one by one.
...
The goal of the \texttt{arduino} class elements is to create and return a vector called \texttt{durations} in which at even indices are the durations of High signals (in \textit{ms}) and at odd indices are the durations of Low signals (in \textit{ms} as well).
...
Since this example output started with the Low signal we cannot place in at the first available index (index 0), because the Low signals are placed at odd indices. In that case a duration of 0ms is placed first (at index 0) and then the proper Low signal duration at the next index 1. These "zero" durations are simply placeholders and they do not spoil the receiving message.
...
Correct placement inside the vector \texttt{durations} is taken care by the following two conditions inside the function \texttt{arduino:read()}:
...
The second condition is handling everything else, so the cases when the current appending index is going to be odd. The situation is now reversed - if the incoming Arduino signal is Low ("L"), we simply append it to the vector and if the incoming signal is High ("H") we first place the "zero" placeholder and then the proper signal duration.
